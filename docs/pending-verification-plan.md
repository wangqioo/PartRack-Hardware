...
- 2026-06-15 烧录后状态：设备可被手机连接，设备名 `VBRK-0000`。
...
| Binding Control Point notify | 绑定表命令响应通过 notify 返回。 | host/model 和测试帧已覆盖；真实 notify 闭环未完成。 | 开启 `7f4b1001-8d1a-4d45-9a4e-2b4a7c000000` notify。 | 写命令后能收到对应状态或数据 notify。 |
| `WRITE_ONE -> READ_ONE` | `WRITE_ONE`、`READ_ONE`、槽位记录编码和解码已实现。 | `tools/ble_gatt_smoke_test.py --print-vectors` 已输出测试帧；host/model 已覆盖。 | 写 `10 01 43 31 32 33 34 35 36 37 00 00 0C 00 00 00 18`，再写 `01 01`。 | 先收到成功状态，再收到 `01 00 01 43 31 32 33 34 35 36 37 00 00 0C 00 00 00 18`。 |
| `READ_ALL` | 全表读取和结束帧已实现。 | host/model 已覆盖结束帧。 | 写 `02` 到 Binding Control Point。 | 收到逐槽记录，最后收到 `02 00 FF`。 |
| `SET_QTY` | 单槽数量修改已实现。 | `tools/binding_table_core_test.c` 和 smoke vectors 已覆盖。 | 在 slot 1 有记录后写 `30 01 2A 00`。 | 收到 `30 00` 状态 notify；再次 `READ_ONE` 时数量为 `0x002A`；Table Info 变化。 |
...
| BLE 加密/配对 | Binding Control Point 配置为 encrypted write。 | 配置存在；手机端行为未完整记录。 | 未配对状态写 Binding CP，按系统提示配对，再重试写入。 | 配对后写入成功；APP 侧可据此实现 authentication/encryption retry。 |
...
|---|---|---|---|
| `tools/ble_gatt_smoke_test.py` | 生成测试帧，并可自动连接、订阅 notify、执行 `WRITE_ONE -> READ_ONE`、`READ_ALL`、`SET_QTY`。 | 当前 macOS 运行环境 CoreBluetooth 报 `BLE is unsupported`，未进入设备扫描。 | 换可访问 BLE 后端的环境运行 `--run-smoke`，保存完整输出。 |
...
3. 全表同步：执行 `READ_ALL`，记录结束帧 `02 00 FF`。
4. 数量修改：执行 `SET_QTY`，确认 `READ_ONE` 和 Table Info 变化。
