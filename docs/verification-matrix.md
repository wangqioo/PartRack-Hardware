...
| `Light Status` 读取 | Hardware verified | `docs/development-plan.md`：空闲状态返回 `0000 00`；2026-06-16 Mac 自动验证 `--run-batch` 中 `FIND` 后返回 `01 0A 00`，OFF 后返回 `00 00 00`；`--run-light-timeout` 中从 `01 0A 00` 倒计时到 `00 00 00`。 | Seeed Studio XIAO nRF52840 Sense，手机 nRF Connect，Mac CoreBluetooth/Bleak。 | 仍需真实灯条、电源门控和 APP 侧订阅 notify 复验。 |
| Device Health Service | Hardware verified | `tools/device_health_test.c` 覆盖 4B payload、电池百分比换算和 flags；2026-06-16 裸 XIAO/peripheral build 已编入 `hwinfo` 和 watchdog driver；2026-06-16 Mac 在 bare 和 peripheral 固件上读取 `7f4b3001-...` 均返回 `64 02 00 00`。 | Seeed Studio XIAO nRF52840 Sense，bare/peripheral PartRack firmware，Mac CoreBluetooth/Bleak。 | 仍需补上电/reset pin/software/watchdog reset 多样本；watchdog enabled/fault 和真实电池 ADC 仍待目标硬件复验。 |
| Binding `WRITE_ONE -> READ_ONE` notify 闭环 | Hardware verified | `docs/xiao-nrf52840-bringup.md`：2026-06-16 `.venv/bin/python tools/ble_gatt_smoke_test.py --run-smoke` 返回 `10 00`，并读回 `01 00 01 43 31 32 33 34 35 36 37 00 00 0C 00 00 00 18`；2026-06-16 peripheral 固件 `--run-batch` 复验通过。 | Seeed Studio XIAO nRF52840 Sense，bare/peripheral PartRack firmware，Mac CoreBluetooth/Bleak。 | 仍需 APP 侧复验 Android 配对/重试流程；仍需 nRF52832 目标板复验。 |
| `READ_ALL` | Hardware verified | `docs/xiao-nrf52840-bringup.md`：2026-06-16 修复为 paced/asynchronous notify 后，`.venv/bin/python tools/ble_gatt_smoke_test.py --run-smoke` 收到 slot 1、后续空槽记录和 `02 00 FF` 结束帧；2026-06-16 peripheral 固件 `--run-batch` 复验通过；`tools/verify_host.sh --host-only` 覆盖 pacer retry/end marker。 | Seeed Studio XIAO nRF52840 Sense，bare/peripheral PartRack firmware，Mac CoreBluetooth/Bleak。 | 仍需 Android APP 侧处理超时/结束帧并复验；仍需 nRF52832 目标板复验。 |
...
| 破坏性绑定表脚本化验证 | Model verified | `tools/ble_gatt_smoke_test.py --run-destructive-binding` 流程已实现；`tools/ble_gatt_smoke_test_test.py` 覆盖 CLEAR/INSERT/SET_QTY/MOVE/REMOVE/FACTORY_RESET validator。 | Python validator；尚未真实 BLE 执行新流程。 | 在测试窗口内对 XIAO 跑 `--run-destructive-binding`，随后 reset 并读回确认。 |
| 灯控超时自动 OFF 脚本化验证 | Hardware verified | 2026-06-16 Mac 自动验证 `tools/ble_gatt_smoke_test.py --run-light-timeout` 通过：`Light Status` 从 `01 0A 00` 倒计时到 `00 00 00`；`tools/ble_gatt_smoke_test_test.py` 覆盖等待 OFF validator。 | Seeed Studio XIAO nRF52840 Sense，bare PartRack firmware，Mac CoreBluetooth/Bleak。 | 接真实灯条后复验物理熄灯；APP 侧订阅 notify 仍需复验。 |
...
| 电池 ADC | Zephyr build verified | `tools/device_health_test.c` 覆盖百分比换算；固件存在 `vbrk-battery-adc` alias 入口，无 alias 时返回 100；2026-06-16 裸 XIAO/peripheral build 通过。 | Host model + Zephyr build；当前 XIAO 裸板无电池 ADC alias。 | 目标板分压回来后添加 alias，做标定和电压下降单调性测试。 |
| 复位原因上报 | Hardware verified | 固件启动时调用 Zephyr `hwinfo_get_reset_cause()` 并通过 Device Health payload 上报；2026-06-16 Mac 在 bare/peripheral 固件上读取 Device Health `64 02 00 00`，其中 reset_reason 为 `0x0002`。 | Seeed Studio XIAO nRF52840 Sense，bare/peripheral PartRack firmware，Mac CoreBluetooth/Bleak。 | 当前只覆盖 reset/pin 类样本；仍需采样上电、software reset、watchdog reset。 |
| 看门狗 | Zephyr build verified | Zephyr watchdog driver 已编入；固件有 `CONFIG_VBRK_WATCHDOG_ENABLE` 开关、8s timeout 和主循环喂狗；默认不开启。 | Zephyr build；尚未启用后做复位闭环。 | 专用验证固件打开 watchdog，验证正常喂狗和故意停止喂狗后的 reset reason。 |
