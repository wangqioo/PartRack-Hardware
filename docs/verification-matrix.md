...
| Device Health Service | Zephyr build verified | `tools/device_health_test.c` 覆盖 4B payload、电池百分比换算和 flags；2026-06-16 裸 XIAO/peripheral build 已编入 `hwinfo` 和 watchdog driver。 | Host model + XIAO Zephyr build；尚未手机实机读取。 | 手机或 Mac 读取 `7f4b3001-...`，记录 payload；补上电/reset pin/software/watchdog reset 样本。 |
...
| 灯控超时自动 OFF 脚本化验证 | Model verified | `tools/ble_gatt_smoke_test.py --run-light-timeout` 流程已实现；`tools/ble_gatt_smoke_test_test.py` 覆盖等待 OFF validator。 | Python validator；尚未真实 BLE 执行新流程。 | 对 XIAO 跑 `--run-light-timeout`，观察 10s 后 `Light Status` 回 `00 00 00`；接灯条后复验熄灯。 |
...
| 复位原因上报 | Zephyr build verified | 固件启动时调用 Zephyr `hwinfo_get_reset_cause()` 并通过 Device Health payload 上报；2026-06-16 build 通过。 | Zephyr build；尚未实机采样不同 reset cause。 | 采样上电、reset pin、software reset、watchdog reset，更新本矩阵。 |
