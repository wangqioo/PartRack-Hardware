...
| Binding `WRITE_ONE -> READ_ONE` notify 闭环 | Hardware required | `tools/ble_gatt_smoke_test.py` 已有测试帧和校验逻辑；`docs/development-plan.md` 明确真实 BLE 烟测阻塞在 macOS CoreBluetooth 后端不可用，未进入设备扫描阶段。 | 当前只有脚本/模型证据，无完整真实 BLE notify 闭环证据。 | 在可用 BLE 后端或手机 nRF Connect 上执行 `WRITE_ONE`、订阅 Binding Control Point notify，再 `READ_ONE` 比对同一 16B 槽位记录。 |
| `READ_ALL` | Hardware required | `tools/ble_gatt_smoke_test.py` 已能校验 `READ_ALL` 结束帧；`docs/firmware-quality-plan.md` 记录 dispatcher model 覆盖 end marker。 | Host/model 覆盖，尚无真实 BLE 端到端记录。 | 真实设备执行 `READ_ALL`，确认返回 25 条记录和 `02 00 FF` 结束帧。 |
| `SET_QTY` | Hardware required | `tools/binding_table_core_test.c` 覆盖 set qty；`tools/ble_gatt_smoke_test.py` 可校验状态 notify。 | Host/model 覆盖，尚无真实 BLE 端到端记录。 | 真实设备执行 `SET_QTY`，确认状态 notify、Table Info notify 和 `table_seq` 变化。 |
