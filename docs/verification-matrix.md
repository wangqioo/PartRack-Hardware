...
| Device Health Service | Hardware verified | `tools/device_health_test.c` 覆盖 4B payload、电池百分比换算和 flags；2026-06-16 裸 XIAO/peripheral build 已编入 `hwinfo` 和 watchdog driver；2026-06-16 Mac 读取 `7f4b3001-...` 返回 `64 02 00 00`。 | Seeed Studio XIAO nRF52840 Sense，bare PartRack firmware，Mac CoreBluetooth/Bleak。 | 仍需补上电/reset pin/software/watchdog reset 多样本；watchdog enabled/fault 和真实电池 ADC 仍待目标硬件复验。 |
...
| 复位原因上报 | Hardware verified | 固件启动时调用 Zephyr `hwinfo_get_reset_cause()` 并通过 Device Health payload 上报；2026-06-16 Mac 读取 Device Health `64 02 00 00`，其中 reset_reason 为 `0x0002`。 | Seeed Studio XIAO nRF52840 Sense，bare PartRack firmware，Mac CoreBluetooth/Bleak。 | 当前只覆盖一次 reset/pin 类样本；仍需采样上电、software reset、watchdog reset。 |
